...
def loaded_bucket(bucket: Bucket) -> Iterable[Bucket]:
    ...
    bucket.upload_file(
        Filename=os.path.join(DATA_DIR, JOBLIB_FILE_KEY),
        Key=JOBLIB_FILE_KEY,
    )
    yield bucket
...
def test_read_dataframe_file_doesnt_exist(loaded_bucket: Bucket) -> None:
    ...
    with pytest.raises(ValueError, match=expected_error):
        _ = s3_utils.read_dataframe(bucket=loaded_bucket.name, key="random_file.elib")


def test_read_dataframe_parquet(loaded_bucket: Bucket) -> None:
    """Tests read_dataframe for a parquet file."""
    # inputformat given
    parquet_actual = s3_utils.read_dataframe(
    ...
    parquet_actual = s3_utils.read_dataframe(
        bucket=loaded_bucket.name, key=PARQUET_FILE_KEY
    )
    pd.testing.assert_frame_equal(PARQUET_EXPECTED, parquet_actual)
...
def test_read_dataframe_csv(loaded_bucket: Bucket) -> None:
    ...
    # inputformat given
    csv_actual = s3_utils.read_dataframe(
        bucket=loaded_bucket.name, key=CSV_FILE_KEY, inputformat="csv"
    )
    pd.testing.assert_frame_equal(CSV_EXPECTED, csv_actual)

    # inputformat inferred from filename
    csv_actual = s3_utils.read_dataframe(bucket=loaded_bucket.name, key=CSV_FILE_KEY)
    pd.testing.assert_frame_equal(CSV_EXPECTED, csv_actual)
...
def test_read_dataframe_tsv(loaded_bucket: Bucket) -> None:
    ...
    # inputformat given
    tsv_actual = s3_utils.read_dataframe(
        bucket=loaded_bucket.name, key=TSV_FILE_KEY, inputformat="tsv"
    )
    pd.testing.assert_frame_equal(TSV_EXPECTED, tsv_actual)

    # inputformat inferred from filename
    tsv_actual = s3_utils.read_dataframe(bucket=loaded_bucket.name, key=TSV_FILE_KEY)
    pd.testing.assert_frame_equal(TSV_EXPECTED, tsv_actual)
...
def test_read_dataframe_txt(loaded_bucket: Bucket) -> None:
    ...
    # inputformat given
    txt_actual = s3_utils.read_dataframe(
        bucket=loaded_bucket.name, key=TXT_FILE_KEY, inputformat="txt"
    )
    pd.testing.assert_frame_equal(TXT_EXPECTED, txt_actual)

    # inputformat inferred from filename
    txt_actual = s3_utils.read_dataframe(bucket=loaded_bucket.name, key=TXT_FILE_KEY)
    pd.testing.assert_frame_equal(TXT_EXPECTED, txt_actual)


def test_read_json(loaded_bucket: Bucket) -> None:
    """Tests read_json."""
    json_actual = s3_utils.read_json(bucket=loaded_bucket.name, key=JSON_FILE_KEY)
...
def test_read_numpy_array(loaded_bucket: Bucket) -> None:
    ...
    np_array_actual = s3_utils.read_numpy_array(
        bucket=loaded_bucket.name, key=NP_ARRAY_FILE_KEY
    )
    np.testing.assert_equal(np_array_actual, NP_ARRAY_EXPECTED)


def test_read_joblib(loaded_bucket: Bucket) -> None:
    """Tests read_numpy_array."""
    joblib_actual = s3_utils.read_joblib(bucket=loaded_bucket.name, key=JOBLIB_FILE_KEY)
...
def test_file_keys_in_bucket(loaded_bucket: Bucket) -> None:
    ...
    file_keys_actual = s3_utils.file_keys_in_bucket(bucket=loaded_bucket.name, key="")
...
def test_file_keys_in_bucket_csv_only(loaded_bucket: Bucket) -> None:
    ...
    file_keys_actual = s3_utils.file_keys_in_bucket(
...
def test_file_exists_in_bucket(loaded_bucket: Bucket) -> None:
    ...
    assert s3_utils.file_exists_in_bucket(bucket=loaded_bucket.name, key=CSV_FILE_KEY)
    assert not s3_utils.file_exists_in_bucket(
        bucket=loaded_bucket.name, key="random_file.csv"
    )
...
def test_file_size(loaded_bucket: Bucket) -> None:
    ...
    size_actual = s3_utils.file_size(bucket=loaded_bucket.name, key=CSV_FILE_KEY)
    assert size_actual in size_expected
    raw_size_actual = s3_utils.file_size(
...
def test_file_size_nonexistent(loaded_bucket: Bucket) -> None:
    ...
    with pytest.raises(ValueError, match=expected_error):
        _ = s3_utils.file_size(bucket=loaded_bucket.name, key="random_file.csv")
